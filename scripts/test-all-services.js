...
async function testService(service) {
  ...
  try {
    ...
    const hasDarkModeClasses =
      html.includes("dark:") || html.includes("data-theme");
    const hasThemeToggle =
      html.includes("theme-toggle") || html.includes("Toggle dark mode");
    const hasNavigation =
      html.includes("Back to Home") || html.includes("navigation");
...
async function runTests() {
  ...
  if (passedServices.length > 0) {
    ...
    passedServices.forEach((r) => {
      console.log(
        `   ${r.name.padEnd(30)} | Dark Mode: ${r.hasDarkMode ? "✓" : "✗"} | Toggle: ${r.hasThemeToggle ? "✓" : "✗"} | Nav: ${r.hasNavigation ? "✓" : "✗"}`,
      );
    });
  }
  ...
  const darkModeSupport = passedServices.filter(
    (r) => r.hasDarkMode && r.hasThemeToggle,
  );
  console.log(
    `   Services with full dark mode: ${darkModeSupport.length}/${passedServices.length}`,
  );

  const missingDarkMode = passedServices.filter(
    (r) => !r.hasDarkMode || !r.hasThemeToggle,
  );
  if (missingDarkMode.length > 0) {
    ...
    missingDarkMode.forEach((r) => {
      console.log(
        `      ${r.name} - Missing: ${!r.hasDarkMode ? "dark classes" : ""} ${!r.hasThemeToggle ? "theme toggle" : ""}`,
      );
    });
  }
