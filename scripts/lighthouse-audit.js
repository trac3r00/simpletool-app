...
function generateSummary(results) {
  ...
  const failedPages = validResults.filter(
    (r) =>
  ...
  if (failedPages.length > 0) {
    ...
    failedPages.forEach((result) => {
      ...
      if (result.scores.bestPractices < 50)
        console.log(`      Best Practices: ${result.scores.bestPractices}%`);
      if (result.scores.seo < 50)
  ...
  const topIssues = Array.from(issueMap.entries())
    .map(([id, count]) => {
      const firstOccurrence = validResults.find((r) =>
        r.issues.some((i) => i.id === id),
      );
...
async function main() {
  ...
  console.log(
    `⚙️  Categories: Performance, Accessibility, Best Practices, SEO`,
  );
