...
async function run() {
  ...
  const workerRuns = pages.map((page, index) => {
    ...
    const runWorker = async () => {
      ...
      while (remaining > 0) {
        ...
        const batch = await page.evaluate(
          async ({
            ...
            const encoders = {
              ...
              base58: (value) => encodeBase58(value),
              jwt: (payload) => {
                const header = encoders.base64url(
            ...
            for (let i = 0; i < count; i++) {
              ...
              for (
                let layerIndex = 0;
                layerIndex < targetLayers;
                layerIndex++
              ) {
                const allowHash =
                  layerIndex === 0 &&
                  baseReversible.includes(expected) &&
                  rng() < 0.1;
                if (allowHash) {
                ...
                if (
                  makeUnsupported &&
                  !unsupportedInjected &&
                  layerIndex === Math.floor(targetLayers / 2)
                ) {
            ...
            for (let idx = 0; idx < cases.length; idx += parallel) {
              ...
              const outputs = await Promise.all(
                slice.map((item) => runPipeline(item.current, 100)),
              );
              outputs.forEach((output, outIndex) => {
                ...
                const isSupported =
                  !item.unsupportedInjected && !item.makeAmbiguous;
                ...
                batchStats.layerDepth.min = Math.min(
                  batchStats.layerDepth.min,
                  item.chain.length,
                );
                batchStats.layerDepth.max = Math.max(
                  batchStats.layerDepth.max,
                  item.chain.length,
                );
                batchStats.layerDepth.sum += item.chain.length;
                batchStats.layerCounts[item.chain.length] =
                  (batchStats.layerCounts[item.chain.length] || 0) + 1;
                ...
                if (success) {
                  ...
                  item.chain.forEach((layer) => {
                    batchStats.failureByLayer[layer] =
                      (batchStats.failureByLayer[layer] || 0) + 1;
                  });
        ...
        workerStats.layerDepth.min = Math.min(
          workerStats.layerDepth.min,
          batch.layerDepth.min,
        );
        workerStats.layerDepth.max = Math.max(
          workerStats.layerDepth.max,
          batch.layerDepth.max,
        );
        ...
        if (workerStats.failures.length < 10) {
          workerStats.failures.push(
            ...batch.failures.slice(0, 10 - workerStats.failures.length),
          );
        }
        ...
        process.stderr.write(
          `worker ${index + 1}: ${workerStats.total}/${perWorker}\n`,
        );
      }
  ...
  workerResults.forEach((batch) => {
    ...
    results.layerDepth.min = Math.min(
      results.layerDepth.min,
      batch.layerDepth.min,
    );
    results.layerDepth.max = Math.max(
      results.layerDepth.max,
      batch.layerDepth.max,
    );
    ...
    if (results.failures.length < 10) {
      results.failures.push(
        ...batch.failures.slice(0, 10 - results.failures.length),
      );
    }
  });

  const avgLayers = results.layerDepth.sum / results.total;
  const supportedRate = results.supportedCases
    ? results.successSupported / results.supportedCases
    : 0;
  ...
  const legacyScore = Math.max(
    ...
    Math.min(
      100,
      Math.round(supportedRate * 100 - avgLayers * 1.2 - unsupportedRate * 15),
    ),
  );
  ...
  const score = Math.max(
    ...
    Math.min(
      100,
      Math.round(
        quality - avgLayersPenalty - unsupportedPenalty - ambiguousPenalty,
      ),
    ),
  );
