...
const handlersSrc = readFileSync(
  join(ROOT, "scripts", "build-routes.js"),
  "utf8",
);
const ROUTES = [...handlersSrc.matchAll(/id:\s*'([^']+)'/g)].map((m) => m[1]);
if (ROUTES.length === 0) {
  console.error(
    "No routes found in scripts/build-routes.js — regex may be stale",
  );
  process.exit(1);
}
...
async function harness() {
  ...
  const skipBtn =
    /change language|theme:|skip to main content|🇺🇸|🇰🇷|🇯🇵|🇪🇸|🇨🇳|🇹🇼|🇫🇷|🇩🇪|🇧🇷|🇻🇳/i;
  ...
  findings.elements.buttons = main.querySelectorAll(
    "button:not([disabled])",
  ).length;
  findings.elements.textInputs = main.querySelectorAll(
    "input:not([type=hidden]):not([type=range]):not([type=checkbox]):not([type=radio]):not([type=color]):not([type=file]):not([readonly]):not([disabled])",
  ).length;
  findings.elements.textareas = main.querySelectorAll(
    "textarea:not([readonly]):not([disabled])",
  ).length;
  findings.elements.selects = main.querySelectorAll(
    "select:not([disabled])",
  ).length;
  findings.elements.ranges = main.querySelectorAll(
    "input[type=range]:not([disabled])",
  ).length;
  findings.elements.checkboxes = main.querySelectorAll(
    "input[type=checkbox]:not([disabled])",
  ).length;
  findings.elements.radios = main.querySelectorAll(
    "input[type=radio]:not([disabled])",
  ).length;
  findings.elements.files = main.querySelectorAll(
    "input[type=file]:not([disabled])",
  ).length;
  findings.elements.colors = main.querySelectorAll(
  ...
  function inferValue(el) {
    ...
    if (/cert|pem/.test(hint))
      return "-----BEGIN CERTIFICATE-----\nMIIDsample\n-----END CERTIFICATE-----";
    if (/jwt|token/.test(hint))
      return "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJ0ZXN0In0.x";
  ...
  const textareas = [
    ...main.querySelectorAll("textarea:not([readonly]):not([disabled])"),
  ];
  ...
  for (const el of [
    ...main.querySelectorAll("input[type=checkbox]:not([disabled])"),
  ].slice(0, 15)) {
    try {
      el.click();
    } catch (e) {
      findings.issues.push(`checkbox ${el.id || "?"}: ${e.message}`);
    }
  }
  ...
  for (const el of main.querySelectorAll("select:not([disabled])")) {
    try {
      ...
      findings.issues.push(`select ${el.id || "?"}: ${e.message}`);
    }
  }
  // Step 5: Color inputs
  for (const el of main.querySelectorAll("input[type=color]:not([disabled])")) {
    try {
      ...
      findings.issues.push(`color ${el.id || "?"}: ${e.message}`);
    }
  }
  ...
  for (const el of main.querySelectorAll("input[type=radio]:not([disabled])")) {
    ...
    try {
      ...
      findings.issues.push(`radio ${el.name || "?"}: ${e.message}`);
    }
  }
  ...
  for (const el of buttons.slice(0, 50)) {
    ...
    const newErrors = (window.__qaErrors || []).slice(
      errorsBefore,
      errorsAfter,
    );
    ...
    if (newErrors.length) {
      findings.issues.push(
        `button "${label}" raised: ${newErrors[0].slice(0, 250)}`,
      );
    }
  }
...
async function qaOne(routeId) {
    ...
    window.addEventListener("error", (e) => {
      queueMicrotask(() => {
        ...
        window.__qaErrors.push(
          `pageerror: ${e.error?.stack || e.error?.message || e.message}`,
        );
      });
    });
  ...
  page.on("console", (msg) => {
    ...
    const t = msg.text();
    if (
      /favicon|Manifest|sentry|adsbygoogle|googletag|gtag|google-analytics|integrity/i.test(
        t,
      )
    )
      return;
  ...
  page.on("pageerror", (err) => {
    ...
    if (
      /NotAllowedError.*clipboard|clipboard.*permission|clipboard.*denied/i.test(
        msg,
      )
    )
      return;
...
console.log(
  `Driving ${ROUTES.length} routes against ${BASE_URL} with concurrency=${concurrency}...`,
);
const results = [];
...
md.push("");
const totalButtons = results.reduce(
  (a, r) => a + (r.interactions?.length || 0),
  0,
);
const totalElements = results.reduce(
  (a, r) =>
    a + (r.elements ? Object.values(r.elements).reduce((s, n) => s + n, 0) : 0),
  0,
);
const totalIssues = results.reduce((a, r) => a + r.issues.length, 0);
const totalPageErrors = results.reduce((a, r) => a + r.pageErrors.length, 0);
const cleanRoutes = results.filter(
  (r) => r.issues.length === 0 && r.pageErrors.length === 0,
).length;
...
md.push(
  `Routes with zero issues + zero pageerrors: **${cleanRoutes} / ${results.length}**`,
);
...
md.push("");
md.push(
  "| Route | Status | Elements | Buttons clicked | Page errors | Issues |",
);
md.push("|---|---|---|---|---|---|");
for (const r of results) {
  const elTotal = r.elements
    ? Object.values(r.elements).reduce((s, n) => s + n, 0)
    : 0;
  const btn = r.interactions?.length || 0;
  const pe = r.pageErrors.length;
  const iss = r.issues.length;
  md.push(
    `| /${r.route} | ${r.status} | ${elTotal} | ${btn} | ${pe} | ${iss} |`,
  );
}
md.push("");
...
if (broken.length === 0) {
  ...
  for (const r of broken) {
    ...
    if (r.pageErrors.length) {
      md.push("Page errors:");
      for (const pe of r.pageErrors.slice(0, 5))
        md.push(`- ${pe.slice(0, 400)}`);
    }
    if (r.consoleErrors.length) {
      md.push("Console errors:");
      for (const ce of r.consoleErrors.slice(0, 5))
        md.push(`- ${ce.slice(0, 300)}`);
    }
    if (r.issues.length) {
      md.push("Issues:");
...
console.log(
  `Clean: ${cleanRoutes}/${results.length} | issues: ${totalIssues} | pageerrors: ${totalPageErrors}`,
);
