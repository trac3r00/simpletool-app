...
const TARGET_URL =
  process.env.DECODER_ALT_URL ||
  "http://localhost:8787/universal-decoder?sim=1";
...
function buildCases() {
  ...
  for (let i = 0; i < basesMedium.length; i++) {
    ...
    cases.push({
      base,
      chain: ["url", "base64url", last === "rot47-bug" ? "rot47" : last],
      input: current,
      expected: base,
    });
  }
  ...
  for (let i = 0; i < 5; i++) {
    ...
    for (const layer of chain)
      current =
        layer === "base64url"
          ? ENCODERS.base64url(current)
          : ENCODERS.reverse(current);
    cases.push({
      base: base,
      chain: ["jwt", ...chain],
      input: current,
