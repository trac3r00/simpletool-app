...
class ClassListStub {
  ...
  contains(name) {
    return this.el._classSet.has(String(name));
  }
  toggle(name, force) {
    const n = String(name);
...
class DocumentStub {
  ...
  importNode(node, deep = false) {
    ...
    if (typeof node.cloneNode === "function")
      return node.cloneNode(Boolean(deep));
    return node;
  }
}
...
function seedFromHtml(doc, html) {
  const re =
    /<(input|textarea|select|button|div|span|p|table|tbody|thead|tr|td|pre)\b([^>]*\bid="[^"]+"[^>]*)>/gi;
  let m;
...
class XMLNode {
  ...
  cloneNode(deep = false) {
    const copy = new XMLNode(
      this.tagName,
      Object.fromEntries(this._attrs),
      this.ownerDocument,
    );
...
class XMLDocumentStub {
  ...
  querySelector(selector) {
    ...
    const sel = String(selector || "")
      .trim()
      .toUpperCase();
    if (sel && this.documentElement.tagName === sel)
      return this.documentElement;
    return this.documentElement.querySelector(selector);
  }
  createTreeWalker(root) {
    return new TreeWalkerStub(root);
  }
}
...
function createVm(toolHtml, marker, extra = {}) {
  ...
  const env = {
    ...
    navigator: {
      clipboard: {
        writeText: async (t) => {
          env.__clipboard = String(t ?? "");
        },
      },
    },
    console: {
      log: (...args) => {
        env.__logs.push(["log", ...args]);
      },
      warn: (...args) => {
        env.__logs.push(["warn", ...args]);
      },
      error: (...args) => {
        env.__logs.push(["error", ...args]);
      },
    },
    __logs: [],
    __clipboard: "",
...
async function runEmailAnalyzer() {
  ...
  while (cases.length < 20) {
    ...
    const dmarc =
      spf === "pass" && dkim === "pass" ? "pass" : i % 2 ? "fail" : "pass";
    const url2 =
      i % 5 === 0 ? `http://203.0.113.${i}` : `https://example.net/path/${i}`;
    cases.push({
      name: "gen-" + i,
      raw: [
  ...
  for (const tc of cases) {
    try {
      input.value = tc.raw;
      await analyze.click();
      const spf = env.document
        .getElementById("spf-result")
        .textContent.toLowerCase();
      const dkim = env.document
        .getElementById("dkim-result")
        .textContent.toLowerCase();
      const dmarc = env.document
        .getElementById("dmarc-result")
        .textContent.toLowerCase();
      assert(
        spf.includes(tc.exp.spf),
        tc.name + ": spf expected " + tc.exp.spf + " got " + spf,
      );
      assert(
        dkim.includes(tc.exp.dkim),
        tc.name + ": dkim expected " + tc.exp.dkim + " got " + dkim,
      );
      assert(
        dmarc.includes(tc.exp.dmarc),
        tc.name + ": dmarc expected " + tc.exp.dmarc + " got " + dmarc,
      );
      passed++;
    } catch (e) {
...
async function runTokenCounter() {
  ...
  while (texts.length < 20)
    texts.push(
      "case-" + texts.length + " " + "lorem ipsum ".repeat(texts.length + 5),
    );
  ...
  return {
    tool: "Token Counter & Cost Estimator",
    total: texts.length + 1,
    passed: passed + (/\$\d+\.\d{2}/.test(costText) ? 1 : 0),
    failures,
  };
}
  ...
  const configs = [
    {
      ...
      task: "Explain this stacktrace and propose fixes.",
      vars: "stacktrace, context",
    },
    {
      target: "claude",
      outFormat: "json",
      role: "You are a security engineer.",
      task: "Generate an incident triage plan.",
      vars: "incident_summary, logs",
    },
    {
      target: "generic",
      outFormat: "checklist",
      role: "You are an SRE.",
      task: "Write a runbook to diagnose latency spikes.",
      vars: "service_name, metrics",
    },
  ];
  while (configs.length < 15)
    configs.push({
      target: "gpt",
      outFormat: "markdown",
      role: "You are a helpful assistant.",
      task: "Summarize: " + configs.length,
      vars: "input",
    });
...
async function runSQLFormatter() {
  ...
  const queries = [
    {
      sql: "select id,email from users where id=1 and active=true order by created_at desc;",
      valid: true,
    },
    {
      sql: "SELECT * FROM t WHERE name = 'unterminated;",
      valid: false,
      expect: "Unclosed single-quoted string",
    },
    {
    ...
    {
      sql: "/* comment without end SELECT 1;",
      valid: false,
      expect: "Unclosed block comment",
    },
  ...
  while (queries.length < 30) {
    ...
    queries.push({
      sql: ok
        ? `SELECT col${i}, col${i + 1} FROM t WHERE a=${i} AND b=${i + 1};`
        : `SELECT col${i} FROM t WHERE a='oops;`,
      valid: ok,
    });
  }
  ...
  for (const [i, q] of queries.entries()) {
    try {
      ...
      if (q.valid) {
        assert(
          /No issues detected|✓/i.test(issuesText),
          "q-" + i + ": expected no issues, got " + issuesText,
        );
      } else if (q.expect) {
        assert(
          issuesText.includes(q.expect),
          "q-" + i + ': expected "' + q.expect + '" got ' + issuesText,
        );
      }
      passed++;
    } catch (e) {
...
async function runEnvVarManager() {
  ...
  const pairs = [
    {
      ...
      exp: { same: 1, changed: 2, onlyA: 0, onlyB: 1 },
    },
    {
      a: 'API_URL=https://dev\nPASSWORD=\"abc12345\"\n',
      b: 'API_URL=https://dev\nPASSWORD=\"abc12345\"\n',
    ...
    },
    {
      a: "export X=1\nY=2 # comment",
      b: "X=1\nY=2\nZ=3",
      exp: { same: 2, changed: 0, onlyA: 0, onlyB: 1 },
    },
  ];
  ...
  for (const [i, p] of pairs.entries()) {
    try {
      ...
      assert(
        same === p.exp.same,
        "pair-" + i + ": same " + same + " != " + p.exp.same,
      );
      assert(
        changed === p.exp.changed,
        "pair-" + i + ": changed " + changed + " != " + p.exp.changed,
      );
      assert(
        onlyA === p.exp.onlyA,
        "pair-" + i + ": onlyA " + onlyA + " != " + p.exp.onlyA,
      );
      assert(
        onlyB === p.exp.onlyB,
        "pair-" + i + ": onlyB " + onlyB + " != " + p.exp.onlyB,
      );
      passed++;
    } catch (e) {
  ...
  return {
    tool: "Environment Variable Manager",
    total: pairs.length,
    passed,
    failures,
  };
...
async function runSVGOptimizer() {
  ...
  while (svgs.length < 10)
    svgs.push(
      `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 10 10"><path d="M${svgs.length} 0h10v10H0z" fill="#${svgs.length}${svgs.length}${svgs.length}"/></svg>`,
    );
  ...
  for (const [i, svg] of svgs.entries()) {
    try {
      ...
      assert(
        outv.includes("<svg"),
        "svg-" + i + ": output should contain <svg",
      );
      assert(
        !outv.toLowerCase().includes("<script"),
        "svg-" + i + ": output should not contain <script",
      );
      assert(
        !outv.toLowerCase().includes("onload="),
        "svg-" + i + ": output should not contain onload=",
      );
      passed++;
    } catch (e) {
  ...
  try {
    ...
    const firstInput = findFirstDescendant(
      colorsBox,
      (n) => n?.tagName === "INPUT",
    );
    ...
    await apply.click();
    assert(
      String(out.value || "").includes("#0000ff"),
      "recolor: expected output to include #0000ff",
    );
    passed++;
  } catch (e) {
...
async function runCSPBuilder() {
  ...
  try {
    ...
    assert(
      ...
      "baseline: should include header name",
    );
    assert(
      v.includes("object-src 'none'"),
      "baseline: should include object-src none",
    );
    passed++;
  } catch (e) {
  ...
  const parseCases = [
    {
      ...
      value:
        "default-src 'self'; script-src 'self' 'unsafe-inline'; object-src 'none'; base-uri 'none';",
  ...
  for (const tc of parseCases) {
    try {
      ...
      assert(
        tc.expect.test(w),
        tc.name + ": expected warning match " + tc.expect,
      );
      passed++;
    } catch (e) {
      failures.push({ name: tc.name, error: String(e?.message || e) });
  ...
  return {
    tool: "CSP Header Builder",
    total: 1 + parseCases.length,
    passed,
    failures,
  };
}
...
async function runSecretScanner() {
  ...
  const cases = [
    ...
    {
      name: "github",
      text: "github_pat_1234567890_abcdefghijklmnopqrstuvwxyz",
      exp: { high: 1 },
    },
    { name: "google", text: "AIza" + "a".repeat(35), exp: { high: 1 } },
    {
      name: "slack",
      text: "xoxb-1234567890-abcdefg-hijklmnop",
      exp: { high: 1 },
    },
    {
      name: "bearer",
      text: "Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0In0.signature",
      exp: { med: 1 },
    },
    { name: "password", text: 'password=\"supersecret\"', exp: { med: 1 } },
    {
      name: "api_key_low",
      text: 'api_key: \"abcdefg1234567890\"',
      includeLow: true,
      exp: { low: 1 },
    },
    {
  ...
  for (const tc of cases) {
    try {
      ...
      if (tc.exp.high)
        assert(
          numText(high) >= tc.exp.high,
          tc.name + ": expected high >= " + tc.exp.high,
        );
      if (tc.exp.med)
        assert(
          numText(med) >= tc.exp.med,
          tc.name + ": expected med >= " + tc.exp.med,
        );
      if (tc.exp.low)
        assert(
          numText(low) >= tc.exp.low,
          tc.name + ": expected low >= " + tc.exp.low,
        );
      if (tc.exp.redacted)
        assert(
          String(redacted.value || "").includes("[REDACTED:"),
          tc.name + ": expected redacted markers",
        );
      passed++;
    } catch (e) {
      failures.push({ name: tc.name, error: String(e?.message || e) });
...
async function main() {
  ...
  results.forEach((r) => {
    ...
    (r.failures || [])
      .slice(0, 3)
      .forEach((f) => console.log("  - FAIL " + f.name + ": " + f.error));
    if ((r.failures || []).length > 3)
      console.log("  - … +" + ((r.failures || []).length - 3) + " more");
  });
