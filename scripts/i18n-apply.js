...
const singleTool = args.includes("--tool")
...
function applyHTMLi18n(content, toolId, strings) {
  ...
  for (const [key, info] of Object.entries(strings)) {
    ...
    if (info.type === "placeholder") {
      ...
      const re = new RegExp(
        `placeholder=["']${escaped}["'](?!\\s*data-i18n)`,
        "g",
      );
      ...
      const escaped = escapeRegex(en);
      const re = new RegExp(
        `(uppercase tracking-wide[^"]*"[^>]*)>(\\s*)${escaped}(\\s*)<`,
        "g",
      );
      const before = modified;
      modified = modified.replace(re, (match, attrs, ws1, ws2) => {
      ...
      const escaped = escapeRegex(en);
      const re = new RegExp(
        `(<(?:p|span)[^>]*class="[^"]*text-(?:sm|xs|surface)[^"]*"[^>]*)>(\\s*)${escaped}(\\s*)</(p|span)>`,
        "g",
      );
      const before = modified;
...
function processFile(toolId) {
  ...
  if (totalChanges > 0) {
    if (dryRun) {
      console.log(
        `  ${filename}: ${totalChanges} changes (HTML: ${html.changeCount}, JS: ${js.changeCount}) [DRY RUN]`,
      );
    } else {
      fs.writeFileSync(filePath, js.content);
      console.log(
        `  ${filename}: ${totalChanges} changes applied (HTML: ${html.changeCount}, JS: ${js.changeCount})`,
      );
    }
  } else {
