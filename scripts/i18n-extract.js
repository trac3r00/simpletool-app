...
function extractHTMLStrings(html, toolId) {
  ...
  const labelClassRe =
    /<label[^>]*class="[^"]*label[^"]*"[^>]*>([\s\S]*?)<\/label>/gi;
  ...
  const descRe =
    /<(?:p|span)[^>]*class="[^"]*text-(?:sm|xs|surface)[^"]*"[^>]*>([^<]{3,})<\/(?:p|span)>/gi;
  ...
  const checkLabelRe =
    /<span[^>]*class="[^"]*text-sm font-medium[^"]*"[^>]*>([^<]+)<\/span>/gi;
...
function extractJSStrings(scriptContent, toolId) {
  ...
  const tplHtmlRe =
    /`[^`]*<(?:span|p|div|td|th|label|h[1-6]|li|option|strong|em)[^>]*>([^<$]{3,})<\//g;
...
if (outFile) {
  ...
  console.log(
    `Extracted ${totalStrings} strings from ${Object.keys(manifest).length} tools → ${outFile}`,
  );
} else {
  console.log(output);
}
...
console.error(
  `  TOTAL: ${totalStrings} strings from ${Object.keys(manifest).length} tools`,
);
