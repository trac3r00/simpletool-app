...
const ROUTE_IDS = [...handlersSrc.matchAll(/id:\s*'([^']+)'/g)].map(
  (m) => m[1],
);
  ...
  try {
    ...
    if (!STATIC_ROUTES.has(routeId) && probe.buttons === 0) {
      result.issues.push({
        severity: "minor",
        kind: "ux",
        detail: "no interactive buttons present",
      });
    }
  } catch (err) {
    result.issues.push({
    ...
    if (result.consoleErrors.length) {
      result.issues.push({
        severity: "major",
        kind: "console",
        detail: `${result.consoleErrors.length} console error(s)`,
      });
    }
    if (result.networkFailures.length) {
      result.issues.push({
        severity: "major",
        kind: "network",
        detail: `${result.networkFailures.length} failed request(s)`,
      });
    }
    if (result.pageError) {
      result.issues.push({
...
const withWorst = results.map((r) => {
  const worst = r.issues.reduce(
    (acc, i) => Math.min(acc, severityOrder[i.severity] ?? 3),
    3,
  );
