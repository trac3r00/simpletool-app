...
test.describe("Home search query parameter", () => {
  test("/?q=json prefills search, filters cards, and has no console errors", async ({
    page,
  }) => {
    const pageErrors = [];
    ...
    page.on("console", (msg) => {
      ...
      if (
        msg.type() === "error" &&
        !/Failed to load resource|net::ERR/.test(text)
      ) {
    ...
    const mismatches = await page.evaluate(() => {
      ...
      cards.forEach((card) => {
        ...
        const matches =
          ...
          desc.includes(q) ||
          tags.includes(q) ||
          renderedName.includes(q) ||
          renderedDesc.includes(q);
  ...
  test("/ shows full grid with empty search box", async ({ page }) => {
    ...
    const visibleCount = await page
      .locator("[data-tool-id]:not(.hidden)")
      .count();
    expect(visibleCount).toBeGreaterThan(0);
  });

  test("/?q=zz-no-tool-match shows empty state with query text", async ({
    page,
  }) => {
