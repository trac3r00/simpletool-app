...
test.describe("DNS Record Reference", () => {
  ...
  test("search filters record types", async ({ page }) => {
    ...
    const visibleCards = page.locator(
      '.record-card:not([style*="display: none"])',
    );
...
test.describe("HTTP Status Reference", () => {
  ...
  test("search input and class filters are visible", async ({ page }) => {
    ...
    await expect(
      page.locator('button.class-filter[data-class="all"]'),
    ).toBeVisible();
    await expect(
      page.locator('button.class-filter[data-class="4xx"]'),
    ).toBeVisible();
  });
...
test.describe("CIDR Calculator", () => {
  ...
  test("host planner calculates ideal prefix", async ({ page }) => {
    ...
    await expect(page.locator("#host-plan-result")).not.toContainText(
...
test.describe("Wireshark Filter Builder", () => {
  ...
  test("protocol reference accordion exists", async ({ page }) => {
    ...
    const detailsCount = await page
      .locator("#protocol-reference details")
      .count();
...
test.describe("Protocol Header Visualizer", () => {
  ...
  test("Ethernet is default protocol", async ({ page }) => {
    ...
    await expect(
      page.locator('.protocol-tab[data-protocol="ethernet"]'),
    ).toHaveClass(/active/);
  });
  ...
  test("hex dump parsing works", async ({ page }) => {
    ...
    await expect(page.locator("#parsed-output-section")).not.toHaveClass(
      /hidden/,
    );
  });
