...
test.describe("Page sweep 2026-05-11 regressions", () => {
  for (const path of ["/", "/qr-code"]) {
    test(`${path} loads local Material Symbols font`, async ({ page }) => {
      ...
      expect(fontResponses.some((response) => response.status() === 404)).toBe(
        false,
      );
      await expect
        .poll(() =>
  ...
  test("ssh-key-generator: RSA produces OpenSSH wire format", async ({
    ...
    await page.locator("#generate-btn").click();
    await expect(page.locator("#results")).toBeVisible();
    await expect(page.locator("#public-key")).toHaveValue(
  ...
  test("htpasswd-generator: Generate entry produces bcrypt output", async ({
    ...
    const strongBtn = page.locator(
      'button:has-text("Generate strong"), button:has-text("Generate password")',
    );
    ...
    await expect(page.locator("#entry-output")).toContainText(
      /admin:\$2[aby]\$/,
    );
    await expect(page.locator("#entry-output")).not.toContainText(
      "No entry yet.",
    );
