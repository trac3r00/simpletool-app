...
test.describe("Ladder Game", () => {
  ...
  test("step 1: player count defaults to 4 with increment/decrement", async ({
    page,
  }) => {
  ...
  test("step 2: preset select populates result inputs", async ({ page }) => {
    ...
    const firstResult = page
      .locator("#results-container .result-input")
      .first();
    const value = await firstResult.inputValue();
    expect(value.length).toBeGreaterThan(0);
  });

  test("step 3: player buttons and result buttons are clickable", async ({
    page,
  }) => {
    // Navigate to step 3
    await page.locator("#btn-step-1-next").click();
...
test.describe("Roulette Wheel", () => {
  ...
  test("add segment increases count", async ({ page }) => {
    const before = await page
      .locator("#segments-list .rw-segment-item")
      .count();
  ...
  test("series mode runs multiple spins", async ({ page }) => {
    ...
    if (
      !(await seriesSection.evaluate((el) => el.classList.contains("open")))
    ) {
      ...
      () => {
        ...
        return (
          ...
          (el.textContent.includes("complete") ||
            el.textContent.includes("Complete") ||
            el.textContent.includes("3 of 3"))
        );
      },
      { timeout: 80000 },
    );
  ...
  test("tournament runs elimination spins and populates ranking", async ({
    page,
  }) => {
    ...
    await page.waitForFunction(
      () =>
        document
          .getElementById("tournament-section")
  ...
  test("presets can be saved and loaded", async ({ page }) => {
    ...
    if (
      !(await presetsSection.evaluate((el) => el.classList.contains("open")))
    ) {
...
test.describe("Marble Roulette", () => {
  ...
  test("speed controls update speed display", async ({ page }) => {
    ...
    await expect(page.locator("#mr-speed-2")).toHaveAttribute(
      "aria-pressed",
      "true",
    );
    ...
    await expect(page.locator("#mr-speed-3")).toHaveAttribute(
      "aria-pressed",
      "true",
    );
  ...
  test("remove winner removes name from textarea and starts new drop", async ({
    page,
  }) => {
  ...
  test("URL params load names and can autostart", async ({ page }) => {
    test.setTimeout(120000);
    await page.goto("/marble-roulette?names=Foo,Bar,Baz&autostart=true", {
