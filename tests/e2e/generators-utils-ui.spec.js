...
test.describe("Generator and utility tools UI interactions", () => {
  ...
  test("mock-data-generator generates output and preview rows", async ({
    page,
  }) => {
    ...
    await expect(page.locator("#output-area")).not.toContainText(
  ...
  test("cron-builder builds expression and next run preview", async ({
    page,
  }) => {
    ...
    await expect(page.locator("#human-readable")).not.toContainText(
      "Invalid cron expression",
    );
    await expect(page.locator("#next-executions li").first()).toBeVisible();
  });

  test("regex-visualizer renders visualization and matches", async ({
    page,
  }) => {
  ...
  test("json-schema-studio generates schema from JSON input", async ({
    page,
  }) => {
    ...
    await page
      .locator("#json-input")
      .fill('{"id":1,"name":"SimpleTool","active":true}');
    await page.locator("#generate-btn").click();
  ...
  test("public-repos-yml-builder generates YAML, audit workflow, and findings", async ({
    ...
    await page
      .locator("#repo-input")
      .fill(
        "trac3r00/simpletool-app team=maintainers cadence=weekly sha=missing branch=protected secrets=ok monetization=todo",
      );
    await page.locator("#policy-monetization-readiness").check();
    await page.locator("#build-inventory").click();

    await expect(page.locator("#repos-yaml-output")).toHaveValue(
      /slug: trac3r00\/simpletool-app/,
    );
    await expect(page.locator("#repos-yaml-output")).toHaveValue(
      /monetization_readiness: todo/,
    );
    await expect(page.locator("#actions-output")).toHaveValue(
      /github\/codeql-action\/init@<codeql_action_init_commit_sha>/,
    );
    await expect(page.locator("#findings-list")).toContainText("SHA pinning");
    await expect(page.locator("#findings-list")).toContainText(
  ...
  test("public-repos-yml-builder imports GitHub public repos API JSON", async ({
    ...
    await expect(page.locator("#actions-output")).toHaveValue(
      /actions\/checkout@<actions_checkout_commit_sha>/,
    );
  });
  ...
  test("public-repos-not-automation builds a manual decision record and checklist", async ({
    ...
    await page
      .locator("#automation-threshold")
      .fill(
        "Three manual runs, written policy, dry-run logs, and rollback owner.",
      );
    await page.locator("#build-decision").click();

    await expect(page.locator("#decision-output")).toHaveValue(
      /Decision: Do not automate yet/,
    );
    await expect(page.locator("#decision-output")).toHaveValue(
      /Repository: trac3r00\/simpletool-app/,
    );
    await expect(page.locator("#decision-output")).toHaveValue(
      /Manual stewardship/,
    );
    await expect(page.locator("#checklist-output")).toHaveValue(
