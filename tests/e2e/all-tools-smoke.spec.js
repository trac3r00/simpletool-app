...
test.describe("All tools smoke", () => {
  ...
  for (const tool of tools) {
    test(`${tool.id}: ${tool.path} loads HTTP 200 with zero page/console errors`, async ({
      page,
    }) => {
      const pageErrors = [];
      const consoleErrors = [];
      ...
      expect(
        response,
        `Expected response for ${tool.path} to be non-null`,
      ).not.toBeNull();
      expect(
        response.status(),
        `Expected ${tool.path} to return HTTP 200`,
      ).toBe(200);
      ...
      expect(
        consoleErrors,
        `Expected zero console errors on ${tool.path}`,
      ).toEqual([]);
    });
  }
});
