...
test.describe('New Tools QA (100+ mock simulations)', () => {
  test('Email Security Analyzer — 20 mock emails', async ({ page }) => {
    const errors = [];
    page.on('console', msg => { if (msg.type() === 'error') errors.push(msg.text()); });

    await page.goto('/email-analyzer');
    ...
    // Generate additional variants to reach 20 cases (different bodies / URLs / headers)
    ...
    for (const tc of cases) {
      await test.step(tc.name, async () => {
        ...
        const urlCount = Number((await page.locator('#url-count').textContent()) || '0');
        expect(urlCount).toBeGreaterThanOrEqual(tc.exp.minUrls);
      });
    }
  ...
  test('Token Counter — 15 mock texts', async ({ page }) => {
    ...
    // Set rates to ensure cost math is exercised
    ...
    for (const [i, t] of texts.entries()) {
      await test.step('text-' + i, async () => {
        ...
        const gptTotal = Number((await page.locator('#gpt-total').textContent())?.replace(/,/g, '') || '0');
        expect(gptIn).toBeGreaterThan(0);
        expect(gptTotal).toBeGreaterThanOrEqual(gptIn);
        lastGpt = Math.max(lastGpt, gptIn);
      });
    }

    const costText = await page.locator('#gpt-cost').textContent();
    expect(costText || '').toMatch(/\$\d+\.\d{2}/);
    expect(lastGpt).toBeGreaterThan(0);
  });

  test('Prompt Template Builder — 10 configs', async ({ page }) => {
    await page.goto('/prompt-template-builder');
    ...
    for (const [i, c] of configs.entries()) {
      await test.step('cfg-' + i, async () => {
        ...
        if (c.target === 'claude') {
          ...
          const sys = await page.locator('#system').inputValue();
          const user = await page.locator('#user').inputValue();
          expect(sys).toContain('You are');
          expect(user).toContain('# Task');
          expect(user).toContain('{{');
        }
      });
  ...
  test('SQL Formatter & Validator — 25 queries', async ({ page }) => {
    ...
    for (const [i, q] of queries.entries()) {
      await test.step('q-' + i, async () => {
        ...
        await page.waitForTimeout(50);
        const issuesVisible = await page.locator('#issues').isVisible().catch(() => false);
        expect(issuesVisible).toBeTruthy();
  ...
  test('Env Var Manager — 15 diffs', async ({ page }) => {
    await page.goto('/env-var-manager');
    ...
    for (const [i, p] of pairs.entries()) {
      await test.step('pair-' + i, async () => {
        ...
        const share = await page.locator('#share').inputValue();
        expect(share).toContain('Env Diff');
      });
    }
  });
  ...
  test('CSP Builder — baseline + parse', async ({ page }) => {
    ...
    await page.waitForTimeout(50);
    const scriptSrc = await page.locator('#dir-script-src').inputValue();
  ...
  test('Secret Scanner — 15 mock leaks', async ({ page }) => {
    await page.goto('/secret-scanner');
