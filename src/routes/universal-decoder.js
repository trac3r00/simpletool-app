      ...
      <div class="bg-white dark:bg-surface-900 border border-surface-200 dark:border-surface-800 rounded-xl shadow-sm p-6 sm:p-8">
        ...
        <div class="grid lg:grid-cols-2 gap-6">
          ...
          <section class="space-y-6">
            ...
            <div id="error-banner" class="hidden p-4 rounded-lg bg-red-50 border border-red-200 text-red-700 dark:bg-red-900/20 dark:border-red-800/60 dark:text-red-200 text-sm font-mono"></div>
    ...
    <script>
      (function() {
        ...
        function renderResults(result) {
          ...
          confidenceBadge.className = 'px-3 py-1 text-xs font-semibold rounded border ' + (steps.length >= 3
            ...
            : steps.length >= 1
              ...
              : 'bg-surface-100 border-surface-200 text-surface-600 dark:bg-surface-900 dark:border-surface-800 dark:text-surface-300');

          const hashLookups = meta.filter((item) => item.type === 'hash-reversed' || item.type === 'hash-lookup');
          const hashSuccesses = hashLookups.filter((item) => item.type === 'hash-reversed' || item.success).length;
          if (hashLookupEl) {
            const lookupClass = hashLookups.length === 0
              ? 'bg-surface-100 border-surface-200 text-surface-600 dark:bg-surface-900 dark:border-surface-800 dark:text-surface-300'
              : hashSuccesses === hashLookups.length
                ? 'bg-green-50 border-green-200 text-green-700 dark:bg-green-900/20 dark:border-green-800 dark:text-green-300'
                : 'bg-yellow-50 border-yellow-200 text-yellow-700 dark:bg-yellow-900/20 dark:border-yellow-800 dark:text-yellow-300';
            hashLookupEl.textContent = '🔍 Hash Lookups: ' + hashSuccesses + '/' + hashLookups.length + ' successful';
            hashLookupEl.className = 'px-3 py-1 text-xs font-semibold rounded border ' + lookupClass;
          }

          renderSteps(steps);
          renderStructures(meta, format);
          renderHashAnalysis(meta);
          renderJsonPreview(format);
          renderQueryParams(format, output);
        ...
        function renderSteps(steps) {
          ...
          if (!steps.length) {
            ...
            empty.className = 'text-surface-500 dark:text-surface-400 font-mono italic';
            empty.textContent = _t('tools.universal-decoder.js.text3', 'No operations run yet.');
          ...
          steps.forEach((step, index) => {
            ...
            card.className = 'rounded-lg border border-surface-200 bg-white p-4 hover:border-surface-300 transition-all dark:border-surface-800 dark:bg-surface-950';

            const title = document.createElement('div');
            title.className = 'flex items-center justify-between text-sm font-semibold text-surface-700 dark:text-surface-200 font-mono mb-2';
            title.innerHTML = '<span>STEP ' + (index + 1) + ' → ' + step.label.toUpperCase() + '</span>';
            card.appendChild(title);

            const desc = document.createElement('p');
            desc.className = 'text-xs text-surface-600 dark:text-surface-400 font-mono mt-1';
            desc.textContent = step.error ? '⚠ FAILED: ' + step.error : step.description;
            card.appendChild(desc);

            if (step.preview) {
              const preview = document.createElement('div');
              preview.className = 'mt-3 text-xs font-mono bg-surface-50 rounded border border-surface-200 p-3 text-surface-700 dark:bg-surface-900/50 dark:border-surface-800 dark:text-surface-200 max-w-full overflow-x-auto whitespace-pre-wrap break-words';
        ...
        function renderStructures(meta, format) {
          structuresEl.innerHTML = '';
          const structures = [];

          meta.forEach((item) => {
            if (item.type === 'jwt') {
              structures.push(renderJWTStructure(item));
            }
          });

          if (format.label === 'URL' && format.parsed instanceof URL) {
            structures.push(renderURLStructure(format.parsed));
          }

          if (!structures.length) {
            const empty = document.createElement('p');
            empty.className = 'text-surface-500 dark:text-surface-400 italic font-mono';
            empty.textContent = _t('tools.universal-decoder.js.text4', 'No structured content detected yet.');
            structuresEl.appendChild(empty);
            return;
          }

          structures.forEach((node) => structuresEl.appendChild(node));
        }

        function renderHashAnalysisLegacy(meta) {
          hashAnalysisEl.innerHTML = '';
          const hashes = meta.filter((item) => item.type === 'hash');
          if (!hashes.length) {
            const empty = document.createElement('p');
            empty.className = 'text-surface-500 dark:text-surface-400 italic font-mono';
            empty.textContent = 'No hashes detected yet.';
            hashAnalysisEl.appendChild(empty);
            return;
          }

          const badgeStyles = {
            MD5: 'bg-red-50 border-red-200 text-red-700 dark:bg-red-900/20 dark:border-red-800 dark:text-red-200',
            SHA1: 'bg-amber-50 border-amber-200 text-amber-700 dark:bg-amber-900/20 dark:border-amber-800 dark:text-amber-200',
            SHA256: 'bg-green-50 border-green-200 text-green-700 dark:bg-green-900/20 dark:border-green-800 dark:text-green-200',
            SHA512: 'bg-blue-50 border-blue-200 text-blue-700 dark:bg-blue-900/20 dark:border-blue-800 dark:text-blue-200'
          };

          hashes.forEach((item) => {
            const card = document.createElement('div');
            card.className = 'rounded-lg border border-surface-200 bg-white p-3 dark:border-surface-800 dark:bg-surface-950';

            const header = document.createElement('div');
            header.className = 'flex flex-wrap items-center gap-2 text-xs font-semibold uppercase tracking-wide';

            const algoBadge = document.createElement('span');
            const algoClass = badgeStyles[item.algorithm] || 'bg-surface-100 border-surface-200 text-surface-600 dark:bg-surface-900 dark:border-surface-800 dark:text-surface-300';
            algoBadge.className = 'px-2 py-0.5 rounded border ' + algoClass;
            algoBadge.textContent = item.algorithm || 'HASH';

            const warning = document.createElement('span');
            warning.className = 'px-2 py-0.5 rounded border bg-red-50 border-red-200 text-red-700 dark:bg-red-900/20 dark:border-red-800 dark:text-red-200';
            warning.textContent = '⚠️ ONE-WAY HASH (irreversible)';

            header.appendChild(algoBadge);
            header.appendChild(warning);
            card.appendChild(header);

            const hashValue = document.createElement('div');
            hashValue.className = 'mt-2 text-xs font-mono text-surface-700 dark:text-surface-200 break-all';
            hashValue.textContent = item.display || item.hash || '';
            card.appendChild(hashValue);

            const message = document.createElement('p');
            message.className = 'mt-2 text-xs text-surface-600 dark:text-surface-300';
            if (item.message) {
              message.textContent = item.message;
            } else if (item.match) {
              message.textContent = (item.algorithm || 'Hash') + ' detected. Found in rainbow table: \\'' + item.match + '\\'.';
            } else {
              message.textContent = (item.algorithm || 'Hash') + ' detected (not in rainbow table - irreversible).';
            }
            card.appendChild(message);

            hashAnalysisEl.appendChild(card);
          });
        }

        function renderHashAnalysis(meta) {
          hashAnalysisEl.innerHTML = '';
          const hashes = meta.filter((item) => item.type === 'hash-reversed' || item.type === 'hash-lookup');
          if (!hashes.length) {
            const empty = document.createElement('p');
            empty.className = 'text-surface-500 dark:text-surface-400 italic font-mono';
            empty.textContent = 'No hashes detected yet.';
          ...
          hashes.forEach((item) => {
            ...
            const isSuccess = item.type === 'hash-reversed' || item.success;
            ...
            status.className = 'px-2 py-0.5 rounded border ' + statusClass;
            status.textContent = isSuccess ? 'RAINBOW HIT' : 'NOT REVERSED';

            header.appendChild(algoBadge);
            header.appendChild(status);
            if (isSuccess) {
              const sourceBadge = document.createElement('span');
              const sourceLabel = (item.source || 'static').toUpperCase();
              sourceBadge.className = 'px-2 py-0.5 rounded border bg-surface-100 border-surface-200 text-surface-600 dark:bg-surface-900 dark:border-surface-800 dark:text-surface-300';
            ...
            const hashValue = document.createElement('div');
            hashValue.className = 'mt-2 text-xs font-mono text-surface-700 dark:text-surface-200 break-all';
            hashValue.textContent = item.hash || '';
            card.appendChild(hashValue);

            const message = document.createElement('p');
            message.className = 'mt-2 text-xs text-surface-600 dark:text-surface-300';
            if (isSuccess) {
              const sourceText = item.source || 'static';
              message.textContent = item.plaintext
                ? 'Reversed via ' + sourceText + ' rainbow table to "' + item.plaintext + '".'
                : 'Reversed via ' + sourceText + ' rainbow table.';
            } else {
              message.textContent = item.error || '⚠️ Hash detected but not found in rainbow table.';
            }
            card.appendChild(message);
        ...
        function renderJWTStructure(meta) {
          const wrapper = document.createElement('div');
          wrapper.className = 'rounded-lg border border-surface-200 bg-white p-4 dark:border-surface-800 dark:bg-surface-950';

          const header = document.createElement('div');
          header.className = 'flex items-center justify-between text-sm font-semibold text-surface-700 dark:text-surface-200 font-mono mb-3';
          header.innerHTML = '<span>JSON WEB TOKEN</span><span class="text-xs text-surface-400" data-i18n="tools.universal-decoder.ui.desc23">HEADER & PAYLOAD DECODED</span>';
          wrapper.appendChild(header);

          const details = document.createElement('div');
          details.className = 'grid sm:grid-cols-2 gap-3 text-xs';
          const rows = [
            { label: 'ALG', value: meta.header.alg || '-' },
            { label: 'TYP', value: meta.header.typ || '-' },
            { label: 'ISS', value: meta.payload.iss || '-' },
            { label: 'SUB', value: meta.payload.sub || '-' }
          ];
          rows.forEach((row) => {
            const item = document.createElement('div');
            const label = document.createElement('p');
          label.className = 'text-surface-500 dark:text-surface-400 font-mono uppercase tracking-wide mb-1';
            label.textContent = row.label;
            const value = document.createElement('p');
          value.className = 'font-mono text-surface-700 dark:text-surface-200';
        ...
        function renderURLStructure(url) {
          ...
          const header = document.createElement('div');
          header.className = 'text-sm font-semibold text-surface-700 dark:text-surface-200 font-mono mb-3 uppercase tracking-wide';
          header.innerHTML = '<span>URL COMPONENTS</span>';
          wrapper.appendChild(header);

          const list = document.createElement('dl');
          list.className = 'grid grid-cols-2 gap-3 text-xs';
          const rows = [
          ...
          term.className = 'text-surface-500 dark:text-surface-400 font-mono uppercase tracking-wide mb-1';
            term.textContent = row.label;
            const desc = document.createElement('dd');
          desc.className = 'font-mono text-surface-700 dark:text-surface-200';
        ...
        function renderJsonPreview(format) {
          if (format.parsed && (format.label === 'JSON Object' || format.label === 'JSON Array')) {
            jsonPreviewEl.textContent = JSON.stringify(format.parsed, null, 2);
          } else {
            jsonPreviewEl.textContent = _t('tools.universal-decoder.js.text5', 'Provide JSON-like content to preview structure.');
          }
        }

        function renderQueryParams(format, rawOutput) {
          queryParamsEl.innerHTML = '';
          const table = document.createElement('div');
          table.className = 'space-y-2';

          let searchParams;
          if (format.label === 'URL' && format.parsed instanceof URL) {
            searchParams = format.parsed.searchParams;
          } else if (format.label === 'Query String') {
            searchParams = new URLSearchParams(rawOutput.trim());
          }

          if (!searchParams || Array.from(searchParams.keys()).length === 0) {
          queryParamsEl.innerHTML = '<p class="text-surface-500 dark:text-surface-400 italic font-mono" data-i18n="tools.universal-decoder.ui.stat14">No query string detected.</p>';
            return;
          }

          searchParams.forEach((value, key) => {
            const row = document.createElement('div');
            row.className = 'flex flex-col sm:flex-row sm:items-center sm:justify-between rounded border border-surface-200 px-3 py-2 bg-surface-50 dark:border-surface-800 dark:bg-surface-900/40';
            const keyEl = document.createElement('span');
            keyEl.className = 'font-mono text-sm text-surface-700 dark:text-surface-200 font-semibold';
            keyEl.textContent = key;
            const valueEl = document.createElement('span');
            valueEl.className = 'font-mono text-xs text-surface-600 dark:text-surface-300 break-all mt-1 sm:mt-0 sm:ml-4';
          ...
          queryParamsEl.appendChild(table);
        }
