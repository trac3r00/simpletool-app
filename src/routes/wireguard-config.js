...
function renderWireguardConfigPage() {
  ...
  const content = `
    ...
    <script src="https://cdnjs.cloudflare.com/ajax/libs/libsodium.js/0.7.13/sodium.min.js" 
      integrity="sha384-Sm9t0L5L5C4f4C2HpLcLqL5/2A8P4A6+8Q4a0D5q5t6h5t6p9y8C5" 
  ...
  const script = `
    <script>
      ...
      function addPeer(template = {}) {
        ...
        document.getElementById('peers-container').appendChild(container.firstElementChild);
      ...
      document.getElementById('generate-qr-btn').addEventListener('click', async () => {
        ...
        container.innerHTML = '<div class="animate-spin h-8 w-8 border-4 border-primary-600 border-t-transparent rounded-full"></div>';
