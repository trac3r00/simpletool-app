...
describe("public-repos-not-automation route rendering", () => {
  ...
  it("renders browser script intent to compose artifacts for every GitHub JSON repo", async () => {
    ...
    expect(text).toContain(
      "tasks.map((task) => buildDecisionRecord(task, selected, threshold)).join",
    );
    expect(text).toContain(
      "tasks.map((task) => buildChecklist(task, selected, threshold)).join",
    );
...
describe("public-repos-not-automation pure helpers", () => {
  it("maps GitHub public repos API JSON arrays into no-automation tasks", () => {
    const tasks = parsePublicReposNoAutomationInput(
      JSON.stringify([
        {
  ...
  it("preserves regular key:value and line input parsing", () => {
    ...
    const line = parsePublicReposNoAutomationInput(
      "https://github.com/trac3r00/docs-site",
    );
  ...
  it("builds decision records and checklists with repo metadata and thresholds", () => {
    const [task] = parsePublicReposNoAutomationInput(
      JSON.stringify([
        {
          full_name: "trac3r00/simpletool-app",
          description: "Browser tools",
    ...
    expect(decision).toContain(
      "3 manual reviews, dry-run evidence, rollback owner.",
    );
    ...
    expect(checklist).toContain(
      "- [ ] Confirm the repository is public: trac3r00/simpletool-app",
    );
    expect(checklist).toContain(
      "- [ ] Confirm manual owner: security maintainers",
    );
    expect(checklist).toContain(
      "- [ ] Re-check reason: Safety or reputation risk",
    );
    expect(checklist).toContain(
      "- [ ] Required threshold: 3 manual reviews, dry-run evidence, rollback owner.",
    );
    expect(checklist).toContain(
      "- [ ] Review repository detail: Language: JavaScript",
    );
