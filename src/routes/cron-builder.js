        ...
        <div class="flex-1 flex flex-col min-h-0 bg-white dark:bg-surface-900 rounded-xl shadow-sm border border-surface-200 dark:border-surface-800 overflow-hidden">
          <div class="border-b border-surface-200 dark:border-surface-800 bg-surface-50/50 dark:bg-surface-950/50">
            <nav class="flex -mb-px" aria-label="Tabs" id="builder-tabs">
              ...
              <button class="tab-btn group inline-flex items-center py-4 px-6 border-b-2 border-transparent font-medium text-sm text-surface-600 hover:text-surface-700 hover:border-surface-300 dark:text-surface-300 dark:hover:text-surface-200 focus:outline-none" data-target="hour" data-tooltip="0-23, which hours to run">
                <span data-i18n="tools.cron-builder.ui.button1">Hour</span>
              </button>
              <button class="tab-btn group inline-flex items-center py-4 px-6 border-b-2 border-transparent font-medium text-sm text-surface-600 hover:text-surface-700 hover:border-surface-300 dark:text-surface-300 dark:hover:text-surface-200 focus:outline-none" data-target="dom" data-tooltip="1-31, which days of the month">
                <span data-i18n="tools.cron-builder.ui.button2">Day</span>
              </button>
              <button class="tab-btn group inline-flex items-center py-4 px-6 border-b-2 border-transparent font-medium text-sm text-surface-600 hover:text-surface-700 hover:border-surface-300 dark:text-surface-300 dark:hover:text-surface-200 focus:outline-none" data-target="month" data-tooltip="1-12, which months">
                <span data-i18n="tools.cron-builder.ui.button3">Month</span>
              </button>
              <button class="tab-btn group inline-flex items-center py-4 px-6 border-b-2 border-transparent font-medium text-sm text-surface-600 hover:text-surface-700 hover:border-surface-300 dark:text-surface-300 dark:hover:text-surface-200 focus:outline-none" data-target="dow" data-tooltip="0-6 (Sun-Sat), which days of week">
          ...
          <div class="p-6 flex-grow overflow-y-auto bg-surface-50/30 dark:bg-surface-950/30">
            ...
            <div id="tab-minute" class="tab-content flex flex-col gap-6">
              ...
              <div id="minute-grid" class="grid grid-cols-6 sm:grid-cols-10 md:grid-cols-12 gap-3 opacity-80 pointer-events-none transition-opacity content-start">
                <!-- Generated by JS -->
              </div>
            </div>
            ...
            <div id="tab-hour" class="tab-content hidden flex flex-col gap-6">
              ...
              <div id="hour-grid" class="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-3 opacity-80 pointer-events-none transition-opacity content-start">
                <!-- Generated by JS -->
              </div>
            </div>
            ...
            <div id="tab-dom" class="tab-content hidden flex flex-col gap-6">
              ...
              <div id="dom-grid" class="grid grid-cols-7 gap-3 opacity-80 pointer-events-none transition-opacity content-start">
                <!-- Generated by JS -->
              </div>
            </div>
            ...
            <div id="tab-month" class="tab-content hidden flex flex-col gap-6">
              ...
              <div id="month-grid" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 opacity-80 pointer-events-none transition-opacity content-start">
                <!-- Generated by JS -->
              </div>
            </div>
            ...
            <div id="tab-dow" class="tab-content hidden flex flex-col gap-6">
              ...
              <div id="dow-grid" class="grid grid-cols-2 sm:grid-cols-4 gap-3 opacity-80 pointer-events-none transition-opacity content-start">
                <!-- Generated by JS -->
              </div>
            </div>
          </div>
        ...
        <div class="flex-1 flex flex-col gap-6 overflow-y-auto pr-1">
          ...
          <div class="bg-white dark:bg-surface-900 rounded-xl shadow-sm border border-surface-200 dark:border-surface-800 p-6">
            <div class="flex items-center justify-between mb-4">
              ...
              <span class="text-xs px-2 py-1 rounded bg-surface-100 dark:bg-surface-800 text-surface-600 dark:text-surface-300" data-i18n="tools.cron-builder.ui.desc20">Local Time</span>
            </div>
            <div class="relative">
          ...
          <div class="bg-white dark:bg-surface-900 rounded-xl shadow-sm border border-surface-200 dark:border-surface-800 p-6">
           ...
           <div class="bg-info-50 dark:bg-info-900/20 rounded-xl p-6 border border-info-100 dark:border-info-800">
             ...
             <ul class="text-sm text-info-700 dark:text-info-200 space-y-1 list-disc list-inside">
    ...
    <script>
      ...
      (function() {
        ...
        function init() {
          ...
          improveRouteLinkCues();
          document.addEventListener('DOMContentLoaded', improveRouteLinkCues, { once: true });

          // Initial Parse
          handleInput();
        }

        function improveRouteLinkCues() {
          document.querySelectorAll('footer p').forEach(paragraph => {
            paragraph.classList.remove('text-surface-500', 'dark:text-surface-500', 'dark:text-surface-400');
            paragraph.classList.add('text-surface-600', 'dark:text-surface-300');
          });

          document.querySelectorAll('footer p a').forEach(link => {
            link.classList.add('underline', 'underline-offset-2');
          });

          document.querySelectorAll('[aria-label="Related tools"] p').forEach(description => {
            description.classList.remove('text-surface-500', 'dark:text-surface-400');
            description.classList.add('text-surface-600', 'dark:text-surface-300');
          });

          const navSearch = document.getElementById('nav-search-btn');
          if (navSearch) {
            navSearch.classList.remove('text-surface-500', 'dark:text-surface-400', 'placeholder-surface-500', 'dark:placeholder-surface-400');
            navSearch.classList.add('text-surface-600', 'dark:text-surface-300', 'placeholder-surface-600', 'dark:placeholder-surface-300');
          }
        }
        ...
        function toggleGrid(part, enabled) {
          ...
          if (enabled) {
            grid.classList.remove('opacity-80', 'pointer-events-none');
          } else {
            grid.classList.add('opacity-80', 'pointer-events-none');
          }
        }
        ...
        function renderGrid(part, start, end, labelFn) {
          ...
          container.setAttribute('aria-label', \`\${PART_LABELS[part]} values\`);

          for (let i = start; i <= end; i++) {
            const label = document.createElement('label');
            label.className = 'flex items-center justify-center w-8 h-8 border border-surface-200 dark:border-surface-700 rounded-full cursor-pointer hover:bg-surface-100 dark:hover:bg-surface-800 transition-colors select-none text-sm text-surface-700 dark:text-surface-200 focus-within:ring-2 focus-within:ring-primary-500 focus-within:ring-offset-2 dark:focus-within:ring-offset-surface-950';
            ...
            span.className = 'peer-checked:font-bold peer-checked:text-primary-700 dark:peer-checked:text-primary-200';
        ...
        function renderRecipes() {
          recipesList.innerHTML = RECIPES.map(r => \`
            <button class="recipe-btn w-full text-left px-4 py-3 rounded-lg hover:bg-surface-50 dark:hover:bg-surface-800 transition-colors flex justify-between items-center group" data-cron="\${r.cron}">
              <span class="text-sm font-medium text-surface-700 dark:text-surface-300 group-hover:text-primary-600 dark:group-hover:text-primary-300">\${r.name}</span>
              <code class="text-xs bg-surface-100 dark:bg-surface-950 px-2 py-1 rounded text-surface-700 dark:text-surface-200 font-mono">\${r.cron}</code>
            </button>
          \`).join('');
        ...
        function switchTab(target) {
          tabs.forEach(t => {
            if (t.dataset.target === target) {
              ...
              t.classList.remove('border-transparent', 'text-surface-600', 'dark:text-surface-300');
            } else {
              t.classList.remove('border-primary-500', 'text-primary-600', 'dark:text-primary-400');
              t.classList.add('border-transparent', 'text-surface-600', 'dark:text-surface-300');
            }
          });
         ...
         function updateNextExecutions() {
           ...
           try {
             ...
             dates.forEach(date => {
               ...
               li.innerHTML = \`
                 <span class="font-mono text-surface-700 dark:text-surface-200">\${formatDate(date)}</span>
                 <span class="text-xs text-surface-600 dark:text-surface-300">(\${timeFromNow(date)})</span>
               \`;
