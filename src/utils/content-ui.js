 ...
 * @returns {string} HTML
 */
export function createEducationalSection(sections) {
  ...
  const items = sections.map((section, i) => `
    <details class="group border-l-4 border-primary-500 dark:border-primary-400 bg-white dark:bg-surface-900 rounded-r-lg shadow-sm"${i === 0 ? ' open' : ''}>
      ...
      <div class="px-5 pb-5 pt-1 text-sm text-surface-700 dark:text-surface-300 leading-relaxed prose dark:prose-invert max-w-none" data-i18n="content.edu.body${i}">
        ${section.content}
      </div>
    </details>
  `).join('');
  ...
  return `
    <div data-section="educational" class="space-y-3" role="region" aria-label="Educational content">
      ${items}
    </div>
  `;
}
