...
export function createEducationalSection(
  ...
  const items = sections
    .map((section, i) => {
      ...
      const headingText =
        t(headingKey, currentLang) !== headingKey
          ? t(headingKey, currentLang)
          : section.title;
      const contentText =
        t(contentKey, currentLang) !== contentKey
          ? t(contentKey, currentLang)
          : section.content;
      return `
...
export function createBreadcrumbs(items, options = {}) {
  ...
  const schemaItems = items.map((item, i) => ({
    ...
    name: item.label,
    ...(item.url
      ? { item: `https://simpletool.app${withLanguageQuery(item.url, lang)}` }
      : {}),
  }));
