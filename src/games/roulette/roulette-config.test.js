...
import {
  buildRouletteToolBadges,
  getRouletteMountMeta,
  getRouletteSectionOrder,
} from "./render.js";
import {
  createRouletteSessionState,
  normalizeRouletteMode,
} from "./runtime.js";
import {
...
describe("roulette module boundary", () => {
  it("exports a config contract with catalog, themes, presets, and modes", () => {
    ...
    expect(Object.keys(rouletteModes)).toEqual(
      expect.arrayContaining(["equal", "weighted"]),
    );
    expect(Object.keys(rouletteSectionMeta)).toEqual(
      expect.arrayContaining(["segments", "mode", "stats", "presets"]),
    );
    expect(
      rouletteBadgeMeta.every((badge) =>
        badge.textKey.startsWith("tools.roulette-wheel.ui."),
      ),
    ).toBe(true);
  });
  ...
  it("builds a serializable boot config for the route", () => {
    ...
    expect(
      bootConfig.sections.find((section) => section.id === "segments")
        ?.defaultOpen,
    ).toBe(true);
    expect(
      bootConfig.presets.every((preset) =>
        preset.labelKey.startsWith("tools.roulette-wheel.ui."),
      ),
    ).toBe(true);
    expect(
      bootConfig.presets.every(
        (preset) => typeof preset.defaultMode === "string",
      ),
    ).toBe(true);
    expect(
      bootConfig.modes.every((mode) =>
        mode.labelKey.startsWith("tools.roulette-wheel.ui."),
      ),
    ).toBe(true);
  });
  ...
  it("provides mount metadata and storage helpers", () => {
    ...
    expect(
      writeStoredRoulettePresets(fakeStorage, rouletteCatalog.storageKey, [
        { name: "alpha", segments: [] },
      ]),
    ).toBe(true);
    expect(
      readStoredRoulettePresets(fakeStorage, rouletteCatalog.storageKey),
    ).toHaveLength(1);
  });
});
